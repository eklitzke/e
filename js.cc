...
namespace e {
  namespace js {
	...
	EventListener::callback_map(const std::string &callback_name, bool use_capture) {
      if (use_capture)
    ...
    EventListener::add(const std::string& callback_name,
                       ...
                       bool use_capture) {
	  std::vector<Handle<Object> *> &callback_list = callback_map(callback_name, use_capture);
